...
public class ProgressDialog extends Dialog implements IProgressReporter
{
    ...
    /**
     * Button at the bottom of the dialog. Used as cancel button at first, later
     * as Finish button if there are error messages to review by the users.
     */
    ...
    protected void createContents()
    {
        ...
        m_shell = PropertyHandler.getModalDialog(getParent());
        display = m_shell.getDisplay();
        final GridLayout gridLayout = new GridLayout();
        gridLayout.verticalSpacing = 10;

        m_shell.setLayout(gridLayout);
        m_shell.setSize(483, 300);
        m_shell.setText("Progress Dialog");

        final Composite composite = new Composite(m_shell, SWT.NONE);
        ...
        // new composite
        cancelComposite = new Composite(m_shell, SWT.NONE);
        ...
        m_textNotification = new Text(cancelComposite, SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER);
        m_textNotification.setLayoutData(descriptionTextData);
        m_textNotification.setEditable(false);

        m_button = new Button(cancelComposite, SWT.NONE);
        m_button.addSelectionListener(new SelectionAdapter()
        {
            @Override
            public void widgetSelected(SelectionEvent e)
        ...
        m_button.setLayoutData(new GridData(78, SWT.DEFAULT));
